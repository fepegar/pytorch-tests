...
class ResectionSideDataset(Dataset):
    def __init__(self, root_dir, padding, resize=64):
        ...
        self.image_transform = self.get_transform(padding, resize, Image.LANCZOS)
        self.label_transform = self.get_transform(padding, resize, Image.NEAREST)
        sample_fields = [
            'image',
            'label',
            'laterality_target',
    ...
    def __getitem__(self, idx):
        ...
        image_tensor = torch.unsqueeze(image_tensor, 0)  # channels x height x width
        ...
        label_tensor = torch.unsqueeze(label_tensor, 0)  # channels x height x width
